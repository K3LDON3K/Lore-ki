...
Článek má název, popis, kategorii, štítky, hlavní obrázek a obsah složený z bloků. Tlačítko **(+)** vloží rovnou **textový blok** — všechno ostatní (nadpisy, seznamy, citace, upozornění, oddělovače, obrázky, vlastní kresby, audio, YouTube, přílohy, reference) se vkládá tlačítky přímo v jeho editoru. Pravé tlačítko na (+) nabídne speciální bloky: stat blok (5e), poznámku DM a šablony. Starší články se samostatnými bloky (nadpis, seznam…) se zobrazují beze změny.
...
Za master heslem. Název a logo aplikace, přehled kampaní a uživatelů, **export zálohy kampaně** do JSON (včetně obrázků), obnovení ze zálohy — buď jako **nová kampaň se zadaným názvem**, nebo **přepisem existující kampaně** (obsah se nahradí zálohou, hráči a jejich přístup zůstávají) — smazání kampaně, vstup do kampaně jako další DM.
